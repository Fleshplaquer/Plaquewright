...
public readonly record struct ResourceRecoveryRequest
{
    ...
    public ResourceRecoveryRequest(
        ...
        double amount)
    {
        if (!double.IsFinite(amount))
        {
            throw new ArgumentOutOfRangeException(
                nameof(amount),
                amount,
                "Resource recovery amount must be finite.");
        }
        ...
        if (amount < 0d)
        {
            throw new ArgumentOutOfRangeException(
                nameof(amount),
                amount,
                "Resource recovery amount cannot be negative.");
        }
    }
